    ...
    <div className="bg-[#26262e] text-white overflow-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden dot-grid">
        ...
        <motion.div className="absolute inset-0 pointer-events-none" style={{ y: heroY }}>
          <div className="absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-violet-600/15 rounded-full filter blur-[140px]" />
          <div className="absolute top-1/3 right-1/4 w-[400px] h-[400px] bg-indigo-600/10 rounded-full filter blur-[100px]" />
          <div className="absolute bottom-1/4 left-1/2 w-[300px] h-[300px] bg-purple-600/8 rounded-full filter blur-[80px]" />
        </motion.div>
            ...
            >
              {[
                ...
                <div key={i} className="text-center group">
                  <s.Icon className="w-5 h-5 text-violet-400/60 mx-auto mb-2 group-hover:text-violet-400 transition-colors" />
                  <div className="text-3xl font-bold text-white">
                    <Counter to={s.to} suffix={s.suffix} />
                  </div>
                  <div className="text-white/35 text-xs mt-1 uppercase tracking-wider">{s.label}</div>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      {/* ── INFO BAR ─────────────────────────────────────────── */}
      <section className="relative border-y border-white/[0.06] py-6 overflow-hidden">
      ...
      <section className="relative py-32 overflow-hidden">
        <div className="grid-lines">{[...Array(7)].map((_, i) => <div key={i} className="grid-line-v" />)}</div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-violet-600/8 rounded-full filter blur-[120px] pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <FadeIn className="text-center mb-16">
            ...
            <p className="text-white/40 max-w-xl mx-auto">Minimally invasive procedures with expert care</p>
          </FadeIn>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {services.map(({ title, desc, Icon, href }, i) => (
              <FadeIn key={i} delay={i * 0.07}>
                <Link href={href} className="group relative card-dark p-7 hover:border-violet-500/30 transition-all duration-300 hover:-translate-y-1 block h-full">
                  ...
                  <div className="relative z-10">
                    <div className="relative w-12 h-12 mb-5">
                      ...
                      <Icon className="absolute inset-0 m-auto w-5 h-5 text-violet-400 group-hover:icon-glow transition-all" />
                    </div>
      ...
      <section className="relative py-32 overflow-hidden border-t border-white/[0.06]">
        <div className="grid-lines">{[...Array(7)].map((_, i) => <div key={i} className="grid-line-v" />)}</div>
        <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-violet-600/8 rounded-full filter blur-[120px] pointer-events-none" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <FadeIn direction="left">
              <div className="relative rounded-3xl overflow-hidden border border-white/[0.08]">
                ...
                >
                  <div className="text-2xl font-bold text-white"><Counter to={15} suffix="+" /></div>
            ...
            <FadeIn direction="right" delay={0.15}>
              ...
              <div className="space-y-4 mb-10">
                {[
                  ...
                  <motion.div
                    key={i}
                    className="flex items-start gap-4 card-dark p-4 hover:border-violet-500/20 transition-all duration-300 group"
      ...
      <section className="relative py-32 overflow-hidden border-t border-white/[0.06]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <FadeIn className="text-center mb-16">
            ...
            <p className="text-white/40 max-w-xl mx-auto">Patient-centred care with a lounge-style medical environment</p>
          </FadeIn>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {whyUs.map(({ Icon, title, desc }, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <motion.div
                  className="card-dark p-7 text-center hover:border-violet-500/25 transition-all duration-300 group h-full"
                  whileHover={{ y: -4 }}
                >
                  <div className="relative w-14 h-14 mx-auto mb-5">
                    <div className="absolute inset-0 rounded-2xl bg-violet-600/10 border border-violet-500/20 group-hover:bg-violet-600/20 transition-colors" />
                    <div className="pulse-ring rounded-2xl opacity-0 group-hover:opacity-100" />
                    <Icon className="absolute inset-0 m-auto w-6 h-6 text-violet-400" />
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {testimonials.map((t, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <motion.div
                  className="card-dark overflow-hidden hover:border-violet-500/20 transition-all duration-300 h-full"
                  whileHover={{ y: -4 }}
                >
                  <div className="relative h-52">
                    ...
                    <div className="absolute bottom-4 left-5">
                      ...
                      <div className="text-white/40 text-xs">{t.treatment}</div>
                    </div>
                  </div>
                  <div className="p-6">
                    <div className="flex gap-0.5 mb-3">
                      {[...Array(5)].map((_, j) => <Star key={j} className="w-3.5 h-3.5 text-violet-400 fill-violet-400" />)}
                    </div>
                    <p className="text-white/50 text-sm leading-relaxed italic">"{t.quote}"</p>
                  </div>
                </motion.div>
              </FadeIn>
            ))}
